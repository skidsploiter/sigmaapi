...
getgenv().request = function(options)
	if options.Headers then
		options.Headers["User-Agent"] = "sigmaAPI/RobloxApp/2.1"
	else
		options.Headers = {["User-Agent"] = "sigmaAPI/RobloxApp/2.1"}
	end
...
getgenv().require = function(scr) -- not mine
	...
	if (type(scr) == "number") then 
		if not game:GetObjects('rbxassetid://' .. scr)[1] then 
			warn("[ sigmaAPI ]: Require failed: invalid asset ID")
			return 
		end
		if typeof(game:GetObjects('rbxassetid://' .. scr)[1]) == "Instance" and game:GetObjects('rbxassetid://' .. scr)[1].ClassName == "ModuleScript" then
			if game:GetObjects('rbxassetid://' .. scr)[1].Name == "MainModule" then 
				if game:GetObjects('rbxassetid://' .. scr)[1].Source ~= "" then 
					...
					warn("[ sigmaAPI ]: Require failed: cant require a modulescript with no code")
				end
			else 
				warn("[ sigmaAPI ]: Require failed: require asset id failed")
			end
		end
		return
	end
...
check("getaffiliateid", function()
    ...
    assert(getgenv().getaffiliateid() == "sigmaAPI", "getaffiliateid function test failed")
end)
...
function Bridge:InternalRequest(body, timeout)
	...
	if body.Url then
		...
		error("[sigmaAPI Error]: Unknown error", 2)
		return
	end
	...
	if success and result then
		error("[sigmaAPI Error]: " .. tostring(result), 2)
	end

	error("[sigmaAPI Error]: Unknown server error", 2)
end
...
function Bridge:request(options)
	...
	return {
		Success = false,
		StatusMessage = "[sigmaAPI Error]: webServer connection failed:  " .. self.serverUrl,
		StatusCode = 599;
...
if not shared.vulnsm then 
	...
	getgenv().rconsoleclear = function()
		Bridge:rconsole("cls") 
		rconsolesettitle("sigmaAPI is NOT fat!")
	end
...
if not shared.vulnsm then 
		...
		if type(real) == "userdata" then
			...
			meta.__index = function(s,k)
				if table.find(vulnFuncTbl, k) then 
					return function()
						error("[ sigmaAPI ]: "..tostring(k).." isn't available.")
					end
	...
	print("[ sigmaAPI ]: Vulns mitigated.")
	shared.vulnsm = true 
end 
...
print("[ sigmaAPI ]: Added functions to the env.")
if not shared.notified then 
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "sigmaAPI Injected",
...
getgenv().IS_sigmaAPI_LOADED = true
